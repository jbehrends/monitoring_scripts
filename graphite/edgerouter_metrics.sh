...
echo "$MetricRoot"."$Host".memory.memTotalReal" $memTotalReal `date +%s`" | nc ${CarbonServer} ${CarbonPort};
echo "$MetricRoot"."$Host".memory.memAvailReal" $memAvailReal `date +%s`" | nc ${CarbonServer} ${CarbonPort};
echo "$MetricRoot"."$Host".memory.memBuffer" $memBuffer `date +%s`" | nc ${CarbonServer} ${CarbonPort};
echo "$MetricRoot"."$Host".memory.memShared" $memShared `date +%s`" | nc ${CarbonServer} ${CarbonPort};
echo "$MetricRoot"."$Host".memory.memCached" $memCached `date +%s`" | nc ${CarbonServer} ${CarbonPort};
